...
// bflCliSanitizeDump() below and the ban list in sitl_lockstep.h):
// systemReset() exits the process on this target.
// ===========================================================================
...
static bool cliEcho = true;         // print captured lines to stdout
static bflCliLineSink_t cliSink;    // harness override for the echo (8B: per-instance state stays small)
static uint32_t cliErrCount;        // output lines carrying the CLI error marker
static char cliErrFirst[192];       // first such line, for error reports

static void cliCapFlush(void)
{
    if (cliCapLen) {
        cliCapBuf[cliCapLen] = '\0';
        // Two error markers cover every rejection: cliPrintErrorVa wraps
        // rejected commands in "###ERROR IN <cmd>: ...###" (unknown
        // names, bad values, parse failures), and the non-interactive
        // CLI prints "ERR_CMD_NA: <line>" for commands this build does
        // not have at all (e.g. 'resource' on SITL)
        if (strstr(cliCapBuf, "###ERROR") || strstr(cliCapBuf, "ERR_CMD_NA")) {
            if (cliErrCount == 0) {
                snprintf(cliErrFirst, sizeof(cliErrFirst), "%s", cliCapBuf);
            }
            cliErrCount++;
        }
        if (cliSink) {
...
char *bflCliSanitizeDump(const char *text, uint32_t len, uint32_t *outLen)
{
    ...
    char *out = malloc((size_t)len * 2 + 64);
    if (!out) {
        return NULL;
    }
    size_t o = 0;
    ...
    while (p < end) {
        ...
        if (lineLen > sizeof(line) - 1) {
            fprintf(stderr, "[cli] dump line longer than %zu bytes dropped\n",
                    sizeof(line) - 1);
            p += lineLen;
            continue;
        }
