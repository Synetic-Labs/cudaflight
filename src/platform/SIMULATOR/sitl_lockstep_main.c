...
// contents: sanitise reboot-class commands out of the text
// (bflCliSanitizeDump, sitl_lockstep.c), feed it through the real CLI
// parser, then save without rebooting. The eeprom file written via
// --eeprom is then a boot-ready config for any backend (CPU file mode,
// GPU RAM preload). Any line the CLI rejects fails the conversion — a
// setting the firmware no longer knows must never silently become a
// compiled default.
...
static int runCliDumpConverter(const char *dumpPath)
{
    ...
    uint32_t cliLen;
    char *cli = bflCliSanitizeDump(text, (uint32_t)size, &cliLen);
    free(text);
    if (!cli) {
    ...
    bflCliExec(cli, cliLen);
    free(cli);

    if (bflCliErrorCount() > 0) {
        fprintf(stderr, "[harness] CLI rejected %u line(s); first: %s\n",
...
// --dump-cli: write the CLI output of 'dump all' (this build's stock
// configuration as durable CLI text) to a file, raw, one line per line.
static FILE *dumpCliOut;

static void dumpCliSink(const char *line)
{
    fprintf(dumpCliOut, "%s\n", line);
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--seconds") == 0 && i + 1 < argc) {
            ...
            cliDumpPath = argv[++i];
        } else if (strcmp(argv[i], "--dump-cli") == 0 && i + 1 < argc) {
            dumpCliPath = argv[++i];
            ...
            fprintf(stderr,
                    ...
                    "       %s --dump-cli OUT.txt   (write this build's stock 'dump all' CLI text)\n",
                    argv[0], argv[0], argv[0]);
            return 1;
        }
    }

    if (cliDumpPath || dumpCliPath) {
        // converter / dump mode: boot one instance, do the one job, exit
        numInstances = 1;
    }
    ...
    for (unsigned k = 0; k < numInstances; k++) {
        ...
        if (dumpCliPath && !eepromBase) {
            // pure dump mode: RAM eeprom, no eeprom.bin side effect
        ...
        if (dumpCliPath) {
            // stock config as CLI text — before bflConfigureArmSwitch and
            // the OSD demo layout, so the dump is purely this build's
            // defaults
            dumpCliOut = fopen(dumpCliPath, "w");
            if (!dumpCliOut) {
                fprintf(stderr, "[harness] cannot write '%s'\n", dumpCliPath);
                return 1;
            }
            bflCliSetSink(dumpCliSink);
            const char dumpCmd[] = "dump all\n";
            bflCliExec(dumpCmd, sizeof(dumpCmd) - 1);
            bflCliSetSink(NULL);
            fclose(dumpCliOut);
            printf("[harness] stock CLI dump written to '%s'\n", dumpCliPath);
            return 0;
        }
