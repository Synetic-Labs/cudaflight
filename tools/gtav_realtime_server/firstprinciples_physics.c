 ...
 *
 * The SCALARS below are a REASONABLE 5" placeholder (sysid later replaces them).
 */
...
// Ground floor clamp. On by default so the drone rests on the ground during the
// firmware boot/arm (rotors stopped) instead of free-falling. The GTA realtime
// path disables it after arming (see fpSetFloorEnabled): in GTA the world terrain
// is the ground and collisions are detected in-game, so a fixed floor at the sim
// origin would just pin the drone to its spawn altitude and stop it descending
// into lower terrain.
...
#define FP_BARO_P0      101325.0
#define FP_BARO_SCALE   8434.0
...
void fpReset(fpSim_t *s, double altitude)
{
    ...
    for (int i = 0; i < 4; i++) s->rotor[i] = rh; // start hovering, not falling
}
...
void fpStep(fpSim_t *s, const float motorsBf[4], double dt)
{
    ...
    double omegaDot[3];
    for (int i = 0; i < 3; i++) omegaDot[i] = (tau[i] - cross[i]) / J[i];
    ...
    // GTA path disables it, so the drone can descend into world terrain.
...
void fpGetImu(const fpSim_t *s, double gyroFrd[3], double accelFrd[3], int32_t *baroPa)
{
    ...
    double fTotal = 0.0;
    for (int i = 0; i < 4; i++) fTotal += FP_KF * s->rotor[i] * s->rotor[i];
    double velBody[3];
    rotInv(s->quat, s->vel, velBody);
