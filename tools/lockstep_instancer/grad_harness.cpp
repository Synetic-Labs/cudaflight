...
//   g++ -O2 grad_harness.cpp -I/opt/cuda/include -L<obj/gpu> -lbfgym -lcuda -o grad_harness
...
extern "C" {
struct bfgym;
bfgym *bfgym_create_eeprom(const char *cubin, uint32_t n, int dev, uint32_t settle, const char *);
void bfgym_destroy(bfgym *);
const char *bfgym_error(void);
uint32_t bfgym_num_envs(bfgym *);
uint64_t bfgym_ctx(bfgym *);
uint32_t bfgym_grid(bfgym *);
uint32_t bfgym_block(bfgym *);
uint64_t bfgym_grad_kernel(bfgym *);
int bfgym_rate_eval(bfgym *);
}
...
int main(int argc, char **argv)
{
    ...
    if (!g) { printf("create failed: %s\n", bfgym_error()); return 1; }

    CK(cuCtxSetCurrent((CUcontext)bfgym_ctx(g)));
    ...
    int re = bfgym_rate_eval(g);
    printf("bfgym_rate_eval (same rebasing, no Enzyme) -> %d (%s)\n", re, re?bfgym_error():"ok");
    CUfunction fn = (CUfunction)bfgym_grad_kernel(g);
    unsigned grid = bfgym_grid(g), block = bfgym_block(g);
