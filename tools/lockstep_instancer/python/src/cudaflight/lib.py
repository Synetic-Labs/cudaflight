...
def load(lib_path: "str | os.PathLike[str] | None" = None) -> ctypes.CDLL:
    ...
    # eeprom_path: boot-ready config image (None boots defaults) — render
    # it from CLI dump text with config.render_eeprom(); see
    # tools/lockstep_instancer/configs/example/
...
def load_cpu(lib_path: "str | os.PathLike[str] | None" = None) -> ctypes.CDLL:
    ...
    # CLI-dump -> eeprom-image renderer (see config.render_eeprom).
    # Guarded: absent before 0.3.5.
    if hasattr(lib, "cpuflight_render_eeprom"):
        lib.cpuflight_render_eeprom.restype = ctypes.c_int
        lib.cpuflight_render_eeprom.argtypes = [
