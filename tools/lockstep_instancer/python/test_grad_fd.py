...
def main():
    lib = load()
    sigs = [
        ("bfgym_grad_fd", ctypes.c_int, [ctypes.c_void_p, ctypes.c_float]),
        ("bfgym_rate_eval", ctypes.c_int, [ctypes.c_void_p]),
        ("bfgym_reset_all", ctypes.c_int, [ctypes.c_void_p]),
        ("bfgym_write_seed_host", ctypes.c_int, [ctypes.c_void_p, ctypes.POINTER(ctypes.c_float)]),
        ("bfgym_grad_read_host", ctypes.c_int, [ctypes.c_void_p, ctypes.POINTER(ctypes.c_float)]),
        ("bfgym_motors_read_host", ctypes.c_int, [ctypes.c_void_p, ctypes.POINTER(ctypes.c_float)]),
        ("bfgym_write_actions_host", ctypes.c_int, [ctypes.c_void_p, ctypes.POINTER(ctypes.c_float)]),
    ]
    ...
    try:
        ...
        act[:, 0] = 0.2   # roll
        act[:, 2] = 0.5   # throttle
        lib.bfgym_write_actions_host(h, fptr(act))

        # --- baseline probe ---
        if lib.bfgym_rate_eval(h) != 0:
            raise SystemExit(f"rate_eval failed: {lib.bfgym_error().decode()}")
        m = np.empty((n, 4), np.float32)
        lib.bfgym_motors_read_host(h, fptr(m))
        print(f"baseline raw motor[] (inst 0): {m[0]}")
        lib.bfgym_reset_all(h)
        lib.bfgym_write_actions_host(h, fptr(act))
        ...
        for i in range(4):
            ...
            seed[:, i] = 1.0
            lib.bfgym_write_seed_host(h, fptr(seed))
            if lib.bfgym_grad_fd(h, ctypes.c_float(EPS)) != 0:
                raise SystemExit(f"grad_fd failed: {lib.bfgym_error().decode()}")
            d = np.empty((n, 4), np.float32)
            lib.bfgym_grad_read_host(h, fptr(d))
            J[i] = d[0]
