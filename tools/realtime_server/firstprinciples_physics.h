 ...
 *
 * Pure C, no firmware deps. The SITL server does the firmware coupling
 * around it (bflGetMotorsNormalised -> fpStep -> fpGetImu ->
 * bflSetGyroAccel/bflSetBaro).
 *
...
// Enable/disable the ground floor clamp (default on, so the drone rests on
// the ground through firmware boot/arm). The realtime server turns it off
// after arming so the drone can descend into world terrain instead of being
// pinned to its spawn altitude — the client's collision handling owns the
// ground.
